    ...
    <div id="login" class="login">
        ...
        <div class="login-form">
            <form @submit.prevent="login">
                ...
                <input v-model="user.email" id="emailID" type="text" name="emailID" placeholder="이메일 아이디를 입력해 주세요.">
        ...
        <div class="errmsg login-errmsg">{{errmsg1}}<br>{{errmsg2}}</div>
        <div class="join">본 서비스 가입을 위해서는 <a href="mailto:bcchoi@aspringcloud.com">bcchoi@aspringcloud.com</a>으로<br> 
            이메일주소, 이름, 소속, 사진, 전화번호를 보내주시기 바랍니다.</div>
...
export default {
    ...
    methods: {
        login () {
            ...
            axios.post("http://115.93.143.2:9103/api/auth/login/",
                { email:this.user.email, password:this.user.pw }
              ).then(res => {
                  this.user= {
