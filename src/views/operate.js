      ...
      socketMsg: function () {
          ...
          // 배차 받기 (200923 신규)
              ...
              if (this.socketMsg.how.function == "call") {
                  ...
                  this.callsArrivalInfo[this.cancelCall.arrivalId].splice(
                      this.callsArrivalInfo[this.cancelCall.arrivalId].indexOf(
                          this.socketMsg.how.uid
                      ),
                      1
                  );
                  ...
                  if (this.callsArrivalInfo[res.data.passed_station]) {
                      for (var uid of this.callsArrivalInfo[res.data.passed_station]) {
                          for (var idx in this.calls) {
                              if (
                                  this.calls[idx].uid == uid &&
                                  this.calls[idx].status == "arrived"
                              )
                                  this.sendCalltoSocket(
                                      uid,
                                      res.data.passed_station,
                                      "end",
                                      idx
                                  );
                          }
